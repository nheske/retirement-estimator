...
if auto:
    ...
    invest_type = default_input(
        "Enter investment number 1. SP500 stocks , 2. 10-yr Treasury Bond, 3. 50% SP500/50% TBond,  4. 40% SP500/50% TBond/10% Cash (default): \n",
        '4').lower()
    ...
    while lifetime_runs_counter < total_runs:
        ...
        if bankrupt == 'yes':
            ...
            if assets_value == max(i for i in lifetime_final_assets):
                print("best outcome so far with ${:,.2f}".format(assets_value) + " in case: " + str(
                    lifetime_runs_counter) + " over " + str(duration) + " years")
